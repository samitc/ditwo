...
template<class T, class Alloc>
Vector<T, Alloc>::Vector() NOEXPECT_NOTHROW_DEFAULT_CONSTRACTUR(T) :Alloc(), dataVal(nullptr), last(nullptr), maxMem(
		nullptr) {
...
checkForReserve(1);
	*last++=Alloc::construct(*this,last,val);
}
template<class T, class Alloc>
...
#define insertVector(num)\
	checkForReserve(num);\
	auto p=dataVal+index;\
	rmove(p,last,p+(num));
template<class T, class Alloc> void Vector<T,Alloc>::insert(uint32 index, const T& val)
{
	insert(index,1,val);
}
template<class T, class Alloc> void Vector<T,Alloc>::insert(uint32 index, T&& val)
{
	insertVector(1);
	++last;
	*p=std::move(val);
...
template<class T, class Alloc> T& Vector<T,Alloc>::operator[](uint32 index) noexcept{
	return *(dataVal+index);
}
template<class T, class Alloc> const T& Vector<T,Alloc>::operator[](uint32 index) const noexcept{
	return *(dataVal+index);
}
template<class T, class Alloc> T& Vector<T,Alloc>::at(uint32 index) noexcept{
	return *(dataVal+index);
}
template<class T, class Alloc> const T& Vector<T,Alloc>::at(uint32 index) const noexcept{
	return *(dataVal+index);
}
template<class T, class Alloc> T& Vector<T,Alloc>::front() noexcept{
	return *(dataVal);
}
template<class T, class Alloc> const T& Vector<T,Alloc>::front() const noexcept{
	return *(dataVal);
}
template<class T, class Alloc> T& Vector<T,Alloc>::back() noexcept{
	return *(last-1);
}
template<class T, class Alloc> const T& Vector<T,Alloc>::back() const noexcept{
	return *(last-1);
}
#endif